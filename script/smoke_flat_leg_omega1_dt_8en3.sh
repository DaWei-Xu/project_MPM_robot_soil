...
# usage:
# mamba run -n env_MPM_robot_soil script/smoke_flat_leg_omega1_dt_8en3.sh

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
SIM_DIR="$ROOT_DIR/MPM-robot-soil-prototype"
OUT_DIR="$ROOT_DIR/data/synthetic_data/smoke_flat_results_omega1_dt_8en3"
SUMMARY_PATH="$OUT_DIR/summary.json"
CSV_PATH="$OUT_DIR/terradyn_flat.csv"
TMP_SIM="$(mktemp "$SIM_DIR/mpm_terradynamics_dt_8en3.XXXXXX.py")"
...
echo "Running flat-leg MPM smoke test at omega=1 with DT=8e-3..."
echo "Output: $OUT_DIR"
